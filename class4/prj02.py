...
class Brick:
    def __init__(self, x, y, width, height, color):
        ...
        self.is_explosive = random.random() < 0.01  # 將機率從 10% 改為 1%
        ...
        for brick in bricks:  # 繪製所有磚塊
            if not brick.hit:  # 如果磚塊沒有被擊中
                ...
                if (
                    ...
                    if brick.is_frozen:
                        self.frozen_timer = 900  # 15秒 * 60幀 = 900幀
...
while True:  # 主程式迴圈
    ...
    if ball.lives <= 0:  # 沒有生命時顯示遊戲結束
        game_over_text = font.render("You Lost!", True, (255, 0, 0))
        text_rect = game_over_text.get_rect(
            center=(bg_x / 2 + shake_offset_x, bg_y / 2 + shake_offset_y)
        )
        ...
        text_rect = thank_you_text.get_rect(
            center=(bg_x / 2 + shake_offset_x, bg_y / 2 + shake_offset_y)
        )
        screen.blit(thank_you_text, text_rect)
        ball.is_moving = False  # 停止球的移動
    else:
        # 分數和生命值顯示
        score_text = font.render(f"Score: {ball.score}", True, (255, 255, 255))
        lives_text = font.render(f"Lives: {ball.lives}", True, (255, 255, 255))
        screen.blit(score_text, (10 + shake_offset_x, 10 + shake_offset_y))
        screen.blit(lives_text, (10 + shake_offset_x, 40 + shake_offset_y))
    ...
    # 繪製冰凍狀態指示
    if ball.frozen_timer > 0:
        frozen_text = font.render("Freezing!", True, (0, 191, 255))
