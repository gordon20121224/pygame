...
# 建立多個平台
platforms = []  # 建立平台列表
platform_count = 10  # 設定平台數量

# 先建立玩家腳下的第一個平台（固定位置）
first_platform = Platform(
    (bg_x - 60) // 2,  # X座標置中（確保在玩家正下方）
    bg_y - 70,  # Y座標在玩家腳下
    60,  # 平台寬度
    10,  # 平台高度
    (255, 255, 255),  # 平台顏色（白色）
...
for i in range(1, platform_count):
    ...
    # 計算平台的Y座標，確保垂直間距為60像素
    platform_y = bg_y - (i * 60) - 50
    # 建立新的平台物件並加入列表
    platforms.append(
        Platform(
            platform_x,  # 隨機X座標
...
while True:
    ...
    player.check_collision(platforms)  # 檢查所有平台的碰撞

    # 繪製遊戲畫面
    for platform in platforms:  # 逐一繪製每個平台
